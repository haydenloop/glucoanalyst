...
function buildSparkline(readings, color, showTarget) {
  ...
  const points = readings.map(r => ({
    x: scaleX(new Date(r.displayTime)),
    y: scaleY(r.value)
  }));
  ...
  const targetTop = scaleY(180);
  const targetBot = scaleY(70);
  const targetBand = showTarget ? `
    <rect x="${pad}" y="${targetTop}" width="${W-pad*2}" height="${targetBot-targetTop}"
      fill="rgba(45,106,79,0.06)" rx="2"/>
    <line x1="${pad}" y1="${targetTop}" x2="${W-pad}" y2="${targetTop}"
      stroke="rgba(45,106,79,0.2)" stroke-width="0.5" stroke-dasharray="3,3"/>
    <line x1="${pad}" y1="${targetBot}" x2="${W-pad}" y2="${targetBot}"
      stroke="rgba(45,106,79,0.2)" stroke-width="0.5" stroke-dasharray="3,3"/>` : '';
    ...
    const content = `
      <div class="connect-page">
        <div class="brand-row">
          ...
          <div class="brand-wordmark">GlucoAnalyst</div>
        </div>
        ...
        <div class="features-row">
          <div class="feature-item">
            ...
            <div class="feature-title">Days analysed</div>
            <div class="feature-desc">Every 5-min reading reviewed</div>
          </div>
          <div class="feature-item">
            <div class="feature-num">AI</div>
            <div class="feature-title">Pattern detection</div>
            <div class="feature-desc">Spots trends you'd never see</div>
          </div>
          <div class="feature-item">
            <div class="feature-num">0</div>
            <div class="feature-title">Medical jargon</div>
            <div class="feature-desc">Plain English. Always.</div>
          </div>
        </div>
      </div>`;
    ...
    <div class="home-page">
      <div class="top-bar">
        <div class="top-wordmark">
          <div style="width:8px;height:8px;background:#2d6a4f;border-radius:50%;flex-shrink:0"></div>
          GlucoAnalyst
        </div>
      ...
      <a href="/analyse" class="analyse-btn">
        <div>
          <div class="analyse-btn-label">Analyse my last 7 days</div>
          <div class="analyse-btn-sub">${getDateRangeLabel()} · ~2,016 readings</div>
        </div>
      ...
      <div class="meta-grid">
        <div class="meta-cell">
          ...
          <div class="meta-value">Dexcom G7</div>
          <div class="meta-sub">CGM system</div>
        </div>
        <div class="meta-cell">
          <div class="meta-label">AI Model</div>
          <div class="meta-value">Claude</div>
          <div class="meta-sub">Sonnet 4.6</div>
        </div>
        <div class="meta-cell">
          <div class="meta-label">Frequency</div>
          <div class="meta-value">5 min</div>
          <div class="meta-sub">Per reading</div>
        </div>
        <div class="meta-cell">
          <div class="meta-label">Period</div>
          <div class="meta-value">7 days</div>
          <div class="meta-sub">${getDateRangeLabel()}</div>
        </div>
      </div>
    </div>`;
  ...
  const content = `
    <div class="loading-page">
      <div class="loading-wordmark">
        <div style="width:6px;height:6px;background:#2d6a4f;border-radius:50%"></div>
        GlucoAnalyst
      </div>
      ...
      <div class="results-page">
        ...
        <div class="score-tir-row">
          ...
          <div class="tir-block">
            ...
            <div class="tir-track">
              <div class="tir-fill" style="width:${tirWidth}%;background:${tirColor}"></div>
            </div>
            ...
            <div class="tir-breakdown">
              <div class="tir-breakdown-item">
                <div class="tir-bd-dot" style="background:#2d6a4f"></div>
                <span>In range (70–180)</span>
                <strong>${tir}%</strong>
              </div>
              <div class="tir-breakdown-item">
                <div class="tir-bd-dot" style="background:#b45309"></div>
                <span>Above range</span>
                <strong>${readings.filter(r=>r.value>180).length > 0 ? Math.round(readings.filter(r=>r.value>180).length/readings.length*100) : 0}%</strong>
              </div>
              <div class="tir-breakdown-item">
                <div class="tir-bd-dot" style="background:#9f1239"></div>
                <span>Below range</span>
                <strong>${readings.filter(r=>r.value<70).length > 0 ? Math.round(readings.filter(r=>r.value<70).length/readings.length*100) : 0}%</strong>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="best-day-block">
          <div class="best-day-header">
            ...
            <div class="best-day-stats">
              <div class="bd-stat">
                <div class="bd-stat-val" style="color:#2d6a4f">${bestDay.tir}%</div>
                <div class="bd-stat-lbl">Time in range</div>
              </div>
              <div class="bd-stat">
                <div class="bd-stat-val">${bestDay.avg}</div>
                <div class="bd-stat-lbl">Avg mg/dL</div>
              </div>
              <div class="bd-stat">
                <div class="bd-stat-val">${bestDay.peak}</div>
                <div class="bd-stat-lbl">Peak</div>
              </div>
            </div>
          </div>
          ${bestDayGraph}
        ...
        <div class="bottom-area">
          ...
          <p class="disclaimer">For informational purposes only. Not a substitute for medical advice from your care team.</p>
        </div>
      </div>`;
    res.send(renderPage(content));
  } catch (err) {
...
app.listen(3000, () => {
  console.log('GlucoAnalyst running at http://localhost:3000');
});
